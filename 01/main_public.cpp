...
public:
  ...
  Expression(const std::string a, const std::string b, const std::string r)
      : strA(std::move(a)), strB(std::move(b)), strR(std::move(r)),
        resolvedDeepth(0) {
    init();
  }

  Expression(const std::string &a, const std::string &b, const std::string &r,
             size_t resolvedDeepth, CharToNumberMapping &resolved,
             CharToNumberOptions &options)
      : strA(a), strB(b), strR(r), resolvedDeepth(resolvedDeepth),
        resolved(resolved), options(options) {}

  uint64_t a() const { return toNumber(strA); }

  uint64_t b() const { return toNumber(strB); }

  uint64_t r() const { return toNumber(strR); }
  ...
  bool isFinished() const { return options.empty(); }
  ...
  void bind(const char ch, uint64_t num) {
    ...
    std::for_each(options.begin(), options.end(),
                  [&num](auto &opt) { opt.second.erase(num); });
  }

  // if ch resolved
  bool isResolved(const char ch) { return resolved.find(ch) != resolved.end(); }
  ...
  std::vector<std::unique_ptr<Expression>> next() {
    ...
    if (toResolved.empty() && (!isFinished())) {
      ...
      auto nextExp = std::make_unique<Expression>(
          strA, strB, strR, resolvedDeepth, resolved, options);
      ret.emplace_back(std::move(nextExp));
    }

    std::set<std::map<uint64_t, char>> tempResolves =
        getTempResolves(toResolved);
...
private:
  ...
  std::set<std::map<uint64_t, char>>
  getTempResolves(const std::set<char> &toResolved) {
    std::set<std::map<uint64_t, char>> tempResolves;
  ...
  // init internal data
  void init() {
    options.emplace(*strA.begin(),
                    std::set<uint64_t>{1, 2, 3, 4, 5, 6, 7, 8, 9});
    options.emplace(*strB.begin(),
                    std::set<uint64_t>{1, 2, 3, 4, 5, 6, 7, 8, 9});
    options.emplace(*strR.begin(),
                    std::set<uint64_t>{1, 2, 3, 4, 5, 6, 7, 8, 9});
    ...
    while (!exps.empty()) {
      ...
      for (const auto &exp : exps) {
        for (auto &nextExp : exp->next()) {
          if (nextExp->isValid()) {
            if (nextExp->a() * nextExp->b() == nextExp->r() &&
                nextExp->isFinished()) {
              std::cout << nextExp->a() << " * " << nextExp->b() << " = "
                        << nextExp->r() << std::endl;
            } else {
